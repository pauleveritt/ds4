# Laguna XS.2 + Streaming + Biased Quant Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** ds4-agent running on a 16 GB Mac Mini at 32k context, backed by Laguna XS.2 streamed from SSD, with quant precision and expert-cache residency biased toward HTML/CSS/JS/TS/Python.

**Architecture:** Add an XS.2 variant to ds4's existing Laguna family support (shape struct + layout validation), route Laguna's Metal MoE dispatch through the existing shared streaming expert cache (GLM integration is the template), then bias two data artifacts: a llama.cpp-built RoutedQ3_K quant using a web/Python imatrix corpus, and an expert hotlist profiled on web/Python agent workloads.
...
- S 2.1 + `--ssd-streaming` must KEEP refusing. Only XS.2 streams.
...
### Task 1: Download official XS.2 GGUFs and record ground-truth facts
...
```bash
curl -s "https://huggingface.co/api/models/poolside/Laguna-XS.2-GGUF/tree/main" | python3 -c "import json,sys; [print(f['path'], f.get('size',0)//2**20,'MiB') for f in json.load(sys.stdin)]"
```
...
curl -L -O "https://huggingface.co/poolside/Laguna-XS.2-GGUF/resolve/main/<Q8_0-file-from-step-1>"
curl -L -O "https://huggingface.co/poolside/Laguna-XS.2-GGUF/resolve/main/<Q4_K_M-file-from-step-1>"
ls -lh laguna-xs*
```
...
./ds4 -m gguf/<Q4_K_M-file> --inspect 2>&1 | head -80
```
...
git commit -m "Record Laguna XS.2 GGUF ground-truth facts"
```

### Task 2: XS.2 shape variant, detection, and head-table generalization
...
**Interfaces:**
- Consumes: xs2-facts.md values (marked `FACTS:` below).
- Produces: `DS4_VARIANT_LAGUNA_XS2` enum value; `DS4_SHAPE_LAGUNA_XS2` struct; shape fields `n_head_global`/`n_head_swa` replacing the hardcoded 48/72 expectation; variant selection by `laguna.block_count`. Later tasks reference the variant constant by exactly this name.
...
static void test_laguna_variant_shapes(void) {
    ...
    /* XS.2 values; FACTS: adjust n_head_global/swa and
     * n_leading_dense to xs2-facts.md before first run. */
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_layer == 40);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_embd == 2048);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_vocab == 100352);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_expert == 256);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_expert_used == 8);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_ff_exp == 512);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.n_ff_dense == 8192);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.rope_scale_factor == 64.0f);
    TEST_ASSERT(DS4_SHAPE_LAGUNA_XS2.rope_orig_ctx == 4096);
}
```
...
Expected: compile error `DS4_SHAPE_LAGUNA_XS2 undeclared` (and `n_head_global` not a member).

- [ ] **Step 3: Implement.** In `ds4.c`:

(a) Variant enum (~line 490): add `DS4_VARIANT_LAGUNA_XS2 = 4,` (use the next free value; check the enum).

(b) Add two fields to `ds4_shape`: `uint32_t n_head_global; uint32_t n_head_swa;`. Set them in `DS4_SHAPE_LAGUNA_S21` (48/72). Leave zero for DeepSeek/GLM shapes (unused there).

(c) New shape struct after `DS4_SHAPE_LAGUNA_S21`:

```c
static const ds4_shape DS4_SHAPE_LAGUNA_XS2 = {
    .name = "Laguna XS.2",
    .family = DS4_MODEL_FAMILY_LAGUNA,
    .variant = DS4_VARIANT_LAGUNA_XS2,
    .n_layer = 40,
    .n_embd = 2048,
    .n_vocab = 100352,
    .n_head = 48,            /* FACTS: max per-layer head count */
    .n_head_global = 48,     /* FACTS: xs2-facts.md item 1 */
    .n_head_swa = 48,        /* FACTS: xs2-facts.md item 1 */
    .n_head_kv = 8,
    .n_head_dim = 128,
    .n_value_dim = 128,
    .n_rot = 64,             /* FACTS: item 3 */
    .n_expert = 256,
    .n_expert_used = 8,
    .n_expert_shared = 1,
    .n_ff_exp = 512,
    ...
    .n_ff_dense = 8192,
    .n_swa = 512,
    .n_leading_dense = 1,    /* FACTS: item 2 */
    .n_rot_swa = 128,        /* FACTS: item 3 */
    .rms_eps = 1.0e-6f,
    ...
    .rope_yarn_attn_factor = 1.4158883083359672f,
...
(d) Variant selection: in the Laguna config-load path (where `DS4_SHAPE_LAGUNA_S21` is installed into `g_ds4_shape`), read `laguna.block_count` FIRST and select: 48 → S21, 40 → XS2, else `ds4_die("unsupported Laguna block_count %u")`.
...
`ds4_laguna_layer_is_swa` (ds4.c:1141) keys on `head_count == DS4_N_HEAD`; verify that still discriminates for XS.2's values from xs2-facts.md. If XS.2 is flat (global == swa == 48), it does NOT discriminate — in that case change the predicate to `(il % 4u) != 0` for the whole family (S 2.1's pattern is also every-4th-global, so this is equivalent and safe; confirm with the S21 array check still passing).
...
git commit -m "Add Laguna XS.2 shape variant and per-variant head tables"
```

### Task 3: XS.2 layout validation + resident bring-up on the laptop
...
**Interfaces:**
- Consumes: Task 2 variant; xs2-facts.md item 6 (Q4_K_M/Q8_0 tensor type maps).
- Produces: XS.2 loads resident on Metal; layout markers `xs2-official-q4km` and `xs2-official-q8` accepted (exact marker mechanism per existing S21 markers at ds4.c:5055 — follow the same detection style: identify by tensor-type pattern, not filename).

- [ ] **Step 1: Extend validation.** In `weights_validate_laguna_layout`, add recognition for the two official XS.2 layouts from xs2-facts.md item 6 (per-layer: routed gate/up/down types, shared types, attention types). Mirror the S21 recipe checks structurally — same loop, same die-with-marker-name on mismatch.
...
```bash
make -j8 && ./ds4 -m gguf/<Q4_K_M-file> -c 4096 -p "def fizzbuzz(n):" -n 64 --nothink
```
...
```bash
./ds4 -m gguf/<Q8_0-file> -c 4096 -p "def fizzbuzz(n):" -n 64 --nothink
```
...
```bash
git add ds4.c
git commit -m "Accept official Laguna XS.2 Q4_K_M and Q8_0 layouts"
```

### Task 4: Quality fixtures for XS.2

**Files:**
- Create: `gguf-tools/quality-testing/data/laguna-xs2/` (continuations)
- Modify: `gguf-tools/quality-testing/collect_official.py` (model entry)

**Interfaces:**
- Consumes: working resident XS.2 (Task 3).
- Produces: fixture continuations used as the quality baseline in Tasks 10 and 13. Reference source: OpenRouter XS.2 if listed (spec unknown #5); otherwise laptop Q8_0 output is the reference — record which in the fixture README.
...
git commit -m "Add Laguna XS.2 quality fixtures and P0 baseline"
```
...
Expected: `ds4: --ssd-streaming for Laguna is only supported for Laguna XS.2`, exit 1.

XS.2 + streaming will fail deeper in (not wired yet) — that's Task 6's job; confirm it gets PAST this guard.

- [ ] **Step 3: `make ds4_test && ./ds4_test` green; commit**

```bash
git add ds4.c
git commit -m "Gate Laguna SSD streaming to the XS.2 variant"
```
...
- Produces: XS.2 generates correct tokens with `--ssd-streaming` on Metal; cache hit/miss counters advance (`g_stream_expert_cache_hits/misses` visible in the existing streaming stats output).
...
- [ ] **Step 5: Budget wiring.** Confirm `ds4.c:30025` (`budget_for_expert_size`) picks up XS.2's `gate_expert_bytes`/`down_expert_bytes` from the first sparse layer's tensors — it reads `layer->ffn_gate_exps->dim[1] * gate_row_bytes`, which is shape-agnostic; verify by log line at startup showing a sane expert count (expect roughly 5 GiB / 1.8 MiB ≈ 2800 experts for Q4_K_M with the automatic budget on the laptop constrained run in Task 7).

- [ ] **Step 6: Smoke run**

```bash
./ds4 -m gguf/<Q4_K_M-file> --ssd-streaming -c 4096 -p "def fizzbuzz(n):" -n 64 --nothink
```
...
git commit -m "Stream Laguna XS.2 routed experts through the Metal expert cache"
```

### Task 7: Streamed-vs-resident correctness gate

**Files:**
- Create: `tests/xs2_stream_ab.sh`
...
```bash
chmod +x tests/xs2_stream_ab.sh && ./tests/xs2_stream_ab.sh
```

Expected: `xs2 stream A/B: OK`. Any mismatch is a Task 6 bug — fix there, re-run, only then proceed.

- [ ] **Step 3: Commit**

```bash
git add tests/xs2_stream_ab.sh
git commit -m "Add XS.2 streamed-vs-resident A/B correctness gate"
```
...
**Files:**
- Modify: `download_model.sh` (add `xs2-q4` target following the `laguna-q4` pattern at lines 6-25/114: repo `poolside/Laguna-XS.2-GGUF`, pinned revision from xs2-facts.md item 5)
...
- [ ] **Step 1: Add and test the download target** (`./download_model.sh xs2-q4` on the laptop; expect skip/verify since the file exists).

- [ ] **Step 2: Deploy to the Mini** (build from this branch on the Mini, copy or re-download the Q4_K_M):

```bash
./ds4-agent -m gguf/<Q4_K_M-file> --ssd-streaming --ctx 32768
```
...
**Files:**
- Create: `gguf-tools/imatrix/dataset/build_laguna_xs2_imatrix_dataset.py`
- Create: `gguf-tools/imatrix/dataset/laguna_xs2_manifest.json` (generated)
...
- Produces: `laguna_xs2_rendered_prompts.txt` (~3M tokens) consumed by Task 10. Composition contract: 55% web/Python, 30% prose/reasoning, 15% shell/C; zero Java/C#/Kotlin/PHP/Go/Rust.
...
```bash
python3 gguf-tools/imatrix/dataset/build_laguna_xs2_imatrix_dataset.py
python3 -c "import json;m=json.load(open('gguf-tools/imatrix/dataset/laguna_xs2_manifest.json'));print(m)"
```
...
git commit -m "Add Python/web-biased Laguna XS.2 imatrix corpus builder"
```

### Task 10: Collect biased imatrix and build the RoutedQ3_K artifact (llama.cpp, out-of-tree)

**Files:**
- Create: `gguf-tools/imatrix/laguna-xs2-README.md` (procedure + exact commands used)
- Produces artifact: `gguf/laguna-xs.2-RoutedQ3_K-biased.gguf` (~13 GiB, not committed)

**Interfaces:**
- Consumes: Task 9 corpus; official BF16 or Q8_0 GGUF as quantize input.
- Produces: the biased GGUF; its exact per-tensor type map recorded in laguna-xs2-README.md for Task 11's validation entry.
...
Verify Laguna support: `./build/bin/llama-cli -m ~/projects/ds4/gguf/<Q4_K_M-file> -p "hi" -n 8` produces tokens.

- [ ] **Step 2: Collect the imatrix against Q8_0 with the biased corpus**

```bash
./build/bin/llama-imatrix -m ~/projects/ds4/gguf/<Q8_0-file> \
  -f ~/projects/ds4/gguf-tools/imatrix/dataset/laguna_xs2_rendered_prompts.txt \
  -o ~/projects/ds4/gguf-tools/imatrix/laguna-xs2-biased.imatrix \
  -c 8192 --chunks 400
```
...
./build/bin/llama-quantize \
  --imatrix ~/projects/ds4/gguf-tools/imatrix/laguna-xs2-biased.imatrix \
  --tensor-type "ffn_gate_exps=q3_k" --tensor-type "ffn_up_exps=q3_k" \
  --tensor-type "ffn_down_exps=q3_k" \
  ~/projects/ds4/gguf/<Q8_0-file> \
  ~/projects/ds4/gguf/laguna-xs.2-RoutedQ3_K-biased.gguf q8_0
```
...
from gguf import GGUFReader
r=GGUFReader('$HOME/projects/ds4/gguf/laguna-xs.2-RoutedQ3_K-biased.gguf')
from collections import Counter
print(Counter(str(t.tensor_type) for t in r.tensors))"
```

- [ ] **Step 4: Write laguna-xs2-README.md** with the exact commands, llama.cpp commit hash, and the tensor map. Commit it.

```bash
git add gguf-tools/imatrix/laguna-xs2-README.md
git commit -m "Document biased XS.2 imatrix collection and Q3_K quantize procedure"
```

### Task 11: Accept the biased layout in ds4 + quality A/B

**Files:**
- Modify: `ds4.c` (`weights_validate_laguna_layout`: add `xs2-routedq3k-q8` recipe per Task 10's tensor map)
- Modify: `gguf-tools/quality-testing/data/laguna-xs2/README.md` (results table)
...
```bash
make -j8 && ./ds4 -m gguf/laguna-xs.2-RoutedQ3_K-biased.gguf -c 4096 -p "def fizzbuzz(n):" -n 64 --nothink
./tests/xs2_stream_ab.sh gguf/laguna-xs.2-RoutedQ3_K-biased.gguf
```

Expected: sane output; `xs2 stream A/B: OK`.
...
git commit -m "Accept biased XS.2 RoutedQ3_K layout with quality A/B results"
```
...
**Files:**
- Create: `gguf-tools/imatrix/laguna-xs2-hotlist-workload.md` (the scripted workload list)
- Produces: `gguf-tools/imatrix/laguna-xs2-biased.hotlist` (text, committed — it is small)
...
- [ ] **Step 1: Script the workload.** ~10 ds4-agent sessions against real Python/web repos (e.g., a Flask app, a vite/TS frontend — local checkouts) with tasks like "add a route/test/component", plus ~20 raw completions over corpus web/Python slices. Record the exact list in laguna-xs2-hotlist-workload.md.

- [ ] **Step 2: Run with the profiler writer active**

```bash
DS4_EXPERT_HOTLIST=gguf-tools/imatrix/laguna-xs2-biased.hotlist \
  ./ds4-agent -m gguf/laguna-xs.2-RoutedQ3_K-biased.gguf -c 32768
```
...
Expected file head: `# ds4 expert hotlist v1`, `# model Laguna XS.2`, sorted `layer expert hits weight` lines.
...
```bash
awk '!/^#/{print $1}' gguf-tools/imatrix/laguna-xs2-biased.hotlist | head -1000 | sort -u | wc -l
```
...
```bash
./ds4 -m gguf/laguna-xs.2-RoutedQ3_K-biased.gguf --ssd-streaming \
  --ssd-streaming-cache-experts 3000 -c 8192 -p "<web/python prompt>" -n 256 --nothink
DS4_METAL_STREAMING_EXPERT_HOTLIST=gguf-tools/imatrix/laguna-xs2-biased.hotlist \
  ./ds4 -m ... (same)
```
...
```bash
git add gguf-tools/imatrix/laguna-xs2-biased.hotlist gguf-tools/imatrix/laguna-xs2-hotlist-workload.md
git commit -m "Profile and record Python/web-biased XS.2 expert hotlist"
```

### Task 13: Bake the hotlist as the XS.2 compiled-in default

**Files:**
- Create: `ds4_streaming_hotlist_laguna_xs2.inc`
...
print(f"/* Laguna XS.2 default streaming expert hotlist. */")
...
```bash
python3 gguf-tools/imatrix/hotlist_to_inc.py gguf-tools/imatrix/laguna-xs2-biased.hotlist > ds4_streaming_hotlist_laguna_xs2.inc
```

In `ds4.c`: add `#include "ds4_streaming_hotlist_laguna_xs2.inc"` at ~1361 and a variant case at ~20841:
...
```bash
git add ds4_streaming_hotlist_laguna_xs2.inc gguf-tools/imatrix/hotlist_to_inc.py ds4.c
git commit -m "Bake Python/web-biased hotlist as the XS.2 streaming default"
```
...
- [ ] **Step 2: Acceptance session**: `./ds4-agent -m gguf/laguna-xs.2-RoutedQ3_K-biased.gguf --ssd-streaming --ctx 32768`; run a scripted multi-tool web/Python task to completion.
...
git commit -m "Record XS.2 Mini acceptance results"
```
