...
// The split-K specialization the kernel above anticipates.  One threadgroup
// still owns one query head, but the key range is striped over eight SIMD
// groups whose independently normalized partials merge in threadgroup memory.
// The serial kernel's cost is a latency chain, not bandwidth: every iteration
// waits on the previous through the online-softmax running maximum, so eight
// stripes buy close to eight times even though the bytes moved are unchanged.
//
...
kernel void kernel_mellum_attention_decode_gqa_split_f16(
    ...
    device float *oh = out + (uint64_t)head * args.head_dim;
    oh[lane]       = merged.x * inv_sum;
    oh[lane + 32u] = merged.y * inv_sum;
    oh[lane + 64u] = merged.z * inv_sum;
    oh[lane + 96u] = merged.w * inv_sum;
}

// Split-K removed the latency chain and left the kernel bandwidth-bound on the
// traffic it issues -- but seven eighths of that traffic is redundant.  Mellum
